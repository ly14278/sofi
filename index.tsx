...
const WORD_TYPES_MAP: { [key in WordType]: string } = {
  Noun: '名词',
  Verb: '动词',
  Adjective: '形容词',
  Adverb: '副词',
...
const WORD_TYPES = Object.keys(WORD_TYPES_MAP) as WordType[];
...
const App = () => {
  ...
  const [numBlanks, setNumBlanks] = useState<number>(10);
  const [selectedWordTypes, setSelectedWordTypes] = useState<Set<WordType>>(new Set(['Noun', 'Verb', 'Adjective']));
...
---END TEXT---
    ...
    try {
      ...
      const resultData = JSON.parse(jsonStr) as ClozeTest;
  ...
  const handleOptionChange = (blankNumber: number, optionKey: string, value: string) => {
    setClozeTest(prev => {
        if (!prev) return null;
  ...
  const handleCorrectAnswerChange = (blankNumber: number, value: 'A' | 'B' | 'C' | 'D') => {
      setClozeTest(prev => {
          if (!prev) return null;
          const newQuestions = prev.questions.map(q => {
              if (q.blankNumber === blankNumber) {
                        ...
                        <div className="options-list editing">
                            ...
                            <div className="correct-answer-edit">
                                ...
                                <select 
                                    ...
                                    onChange={(e) => handleCorrectAnswerChange(q.blankNumber, e.target.value as 'A'|'B'|'C'|'D')}
                                >
